...
class ConfigAdminSupportTest {
    ...
    void testStart() throws InvalidSyntaxException {
        ...
        assertEquals(1, bundleContext.getServiceReferences(ManagedService.class, filter).size());

        filter = String.format("(%s=%s)", Constants.SERVICE_PID, LogConstants.FACTORY_PID_WRITERS);
        assertEquals(1, bundleContext.getServiceReferences(ManagedServiceFactory.class, filter).size());
    ...
    void testStop() throws InvalidSyntaxException {
        ...
        BundleContext bundleContext = context.bundleContext();
        String filter = String.format("(%s=%s)", Constants.SERVICE_PID, LogConstants.PID);
        assertEquals(0, bundleContext.getServiceReferences(ManagedService.class, filter).size());

        filter = String.format("(%s=%s)", Constants.SERVICE_PID, LogConstants.FACTORY_PID_WRITERS);
        assertEquals(0, bundleContext.getServiceReferences(ManagedServiceFactory.class, filter).size());
